...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    foreach ($_POST as $key => $value) {
        ...
        foreach ($ban as $banish) {
            if ($banish[1] == $value) { // Utiliser $value au lieu de $key pour l'email de l'utilisateur
                $is_banned = TRUE;
                break;
            }
        }
        ...
        if ($is_banned) {
            ...
            foreach ($ban_file_contents as $line) {
                ...
                if ($line_data[1] != $value) { // Utiliser $value au lieu de $key pour l'email de l'utilisateur
                    $new_contents[] = $line;
                }
            }
